...
public static class BaseConvert
{
    ...
    public static string ToBase32Hex(byte[]? bytes)
    {
        ...
        if (arrayIndex != charCount) returnArray[arrayIndex++] = ValueToChar(nextChar);
    ...
    {
        if (string.IsNullOrEmpty(base32)) throw new ArgumentNullException(nameof(base32));
    ...
    private static int CharToValue(char c)
    {
        ...
        //uppercase letters
        if (value < 87 && value > 64) return value - 55;
        //numbers 0-9
        if (value < 58 && value > 47) return value - 48;
        //lowercase letters
        if (value < 119 && value > 96) return value - 87;
