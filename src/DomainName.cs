...
public class DomainName : IEquatable<DomainName>
{
    ...
    public bool Equals(DomainName? that)
    {
        ...
        var n = labels.Count;
        if (n != that.labels.Count) return false;
        for (var i = 0; i < n; ++i)
    ...
    public static DomainName Join(params DomainName[] names)
    {
        ...
        foreach (var name in names) joinedName.labels.AddRange(name.Labels);
        return joinedName;
    }
    ...
    public bool IsSubdomainOf(DomainName? domain)
    {
        ...
        if (labels.Count <= domain.labels.Count) return false;
    ...
    private void Parse(string name)
    {
        ...
        if (label.Length > 0) labels.Add(label.ToString());
    }
