...
/// </param>
public class PresentationReader(TextReader text)
{
    private static readonly DateTime UnixEpoch = new(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
    private DomainName? defaultDomainName;
    private TimeSpan? defaultTTL;
    private bool eolSeen;
    ...
    /// </summary>
    private bool tokenStartsNewLine;
    ...
    public ResourceRecord? ReadResourceRecord()
    {
        ...
        if (domainName == null) throw new InvalidDataException("Missing resource record name.");
        ...
        if (ttl.HasValue) resource.TTL = ttl.Value;
    ...
    private string ReadToken(bool ignoreEscape = false)
    {
        ...
        while ((c = text.Read()) >= 0)
        {
            ...
            if (c == '\\')
            {
                if (ignoreEscape)
                {
                    if (sb.Length == 0) tokenStartsNewLine = previousChar == '\r' || previousChar == '\n';
                    sb.Append((char)c);
                    previousChar = c;
