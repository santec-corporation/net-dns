...
/// </param>
public class PresentationWriter(TextWriter text)
{
    ...
    {
        if (!Enum.IsDefined(typeof(DnsType), value)) text.Write("TYPE");
        text.Write(value);
        if (appendSpace)
            WriteSpace();
    }
    ...
    {
        if (!Enum.IsDefined(typeof(DnsClass), value)) text.Write("CLASS");
        text.Write(value);
        if (appendSpace)
            WriteSpace();
    }
