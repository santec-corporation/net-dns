...
public partial class NameServer : IResolver
{
        ...
        CancellationToken cancel = default)
    {
        ...
        if (response.Answers.Count > 0) response.Status = MessageStatus.NoError;

        // Remove duplicate records.
        if (response.Answers.Count > 1) response.Answers = response.Answers.Distinct().ToList();
    ...
    /// </remarks>
    public async Task<Message> ResolveAsync(Question question, Message? response = null,
        CancellationToken cancel = default)
    {
    ...
    protected Task<bool> FindAnswerAsync(Question question, Message response, CancellationToken cancel)
    {
        ...
        if (!Catalog.TryGetValue(question.Name!, out var node)) return Task.FromResult(false);
    ...
    private SOARecord? FindAuthority(DomainName? domainName)
    {
        ...
        while (name != null)
        {
            if (Catalog.TryGetValue(name, out var node))
            {
    ...
    private void AddAdditionalRecords(Message response)
    {
        ...
        if (extras.Answers.Count > 0) AddAdditionalRecords(response);
    }
